...
import plugin.safe.cloud.seal.download.SealDownloadManager;


/**
 * List adapter for Cursors returned by {@link SealDownloadManager}.
 */
public class DownloadAdapter extends CursorAdapter {
	...
	mIdColumnId = cursor.getColumnIndexOrThrow(SealDownloadManager.COLUMN_ID);
	mTitleColumnId = cursor
		.getColumnIndexOrThrow(SealDownloadManager.COLUMN_TITLE);
	...
	mTotalBytesColumnId = cursor
		.getColumnIndexOrThrow(SealDownloadManager.COLUMN_TOTAL_SIZE_BYTES);
	mCurrentBytesColumnId = cursor
		.getColumnIndexOrThrow(SealDownloadManager.COLUMN_BYTES_DOWNLOADED_SO_FAR);
	mMediaTypeColumnId = cursor
		.getColumnIndexOrThrow(SealDownloadManager.COLUMN_MEDIA_TYPE);
	mDateColumnId = cursor
		.getColumnIndexOrThrow(SealDownloadManager.COLUMN_LAST_MODIFIED_TIMESTAMP);
    }
	...
	boolean indeterminate = status == SealDownloadManager.STATUS_PENDING;
	...
	if (status == SealDownloadManager.STATUS_FAILED
		|| status == SealDownloadManager.STATUS_SUCCESSFUL) {
...
//		.isDownloadSelected(downloadId));
	...
	switch (status) {
	case SealDownloadManager.STATUS_FAILED:
	...
	case SealDownloadManager.STATUS_PAUSED:
	    if (mCursor.getInt(mReasonColumnId) == SealDownloadManager.PAUSED_QUEUED_FOR_WIFI) {
	...
	if (list.size() == 0) {
	    ...
	    iconView.setImageResource(R.drawable.seal_default_image);
	} else {
