...
import plugin.safe.cloud.seal.album.AlbumGridViewAdapter;
import plugin.safe.cloud.seal.album.AlbumHelper;
import plugin.safe.cloud.seal.album.ImageBucket;
 ...
 */
public class AlbumActivity extends DLBasePluginActivity {
	// 显示手机里的所有图片的列表控件
...
//				intent.setClass(that, GalleryActivity.class);
...
//			bundle.putse
			resultIntent.putExtra("image_shot", tempSelectBitmap.get(0));
	...
	private class BackListener implements OnClickListener {
		public void onClick(View v) {
//			intent.setClass(that, ImageFile.class);
//			startActivity(intent);
		}
	}
...
//			startActivity(intent);
	...
	private void init() {
		...
		gridImageAdapter = new AlbumGridViewAdapter(that, dataList, tempSelectBitmap);
			...
			public void onItemClick(final ToggleButton toggleButton, int position, boolean isChecked, Button chooseBt) {
				if (tempSelectBitmap.size() >= num) {
					...
					if (!removeOneData(dataList.get(position))) {
						Toast.makeText(that, "超过最大选择数量", Toast.LENGTH_SHORT).show();
					}
					return;
				}
				if (isChecked) {
