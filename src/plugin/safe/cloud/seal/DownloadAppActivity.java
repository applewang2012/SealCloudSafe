...
import plugin.safe.cloud.seal.download.SealDownloadManager.Request;
import plugin.safe.cloud.seal.downloadui.DownloadAdapter;
import plugin.safe.cloud.seal.downloadui.DownloadSelectListener;
...
public class DownloadAppActivity extends BaseActivity{
	...
	private void initDownloadAdapter(){
		...
		mSizeSortedAdapter = new DownloadAdapter(that, mSizeSortedCursor,new DownloadSelectListener() {
	...
	private void updateDownloadStatus(int status){
		...
		case SealDownloadManager.STATUS_SUCCESSFUL:
			...
			CommonUtil.installApk(that, downloadfile);
			break;
		default:
