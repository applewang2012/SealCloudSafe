		...
		mofifyButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (mOldPassword == null || mOldPassword.equals("")){
					Toast.makeText(that, getString(R.string.old_pwd_not_null), Toast.LENGTH_SHORT).show();
					return;
				}
				if (mNewPassword == null || mNewPassword.equals("")){
					Toast.makeText(that, getString(R.string.new_pwd_not_null), Toast.LENGTH_SHORT).show();
					return;
				}
				if (mNewPasswordConfirm == null || mNewPasswordConfirm.equals("")){
					Toast.makeText(that, getString(R.string.new_again_pwd_not_null), Toast.LENGTH_SHORT).show();
					return;
				}
				if (!mNewPassword.equals(mNewPasswordConfirm)){
					Toast.makeText(that, getString(R.string.twice_pwd_not_same), Toast.LENGTH_SHORT).show();
					return;
				}
				showLoadingView();
				modifyUserPassword();
	...
	private Handler mHandler = new Handler(){
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == 100){
			    ...
			    editor.commit();
			    Toast.makeText(that, getString(R.string.modify_pwd_success), Toast.LENGTH_SHORT).show();
				...
				dismissLoadingView();
				Toast.makeText(that, getString(R.string.modify_pwd_failed), Toast.LENGTH_SHORT).show();
			}
