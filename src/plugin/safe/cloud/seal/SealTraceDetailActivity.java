...
public class SealTraceDetailActivity extends BaseActivity {
	...
	private AddSealTraceActivity.GridAdapter adapter;
	...
	public void Init() {
		...
		mAdapter = new UniversalAdapter<SealTraceInfo>(that, R.layout.aty_item_published_grida, mDataList) {
			...
			public void convert(UniversalViewHolder holder, SealTraceInfo info) {
				...
				Picasso.with(that).load(info.GetSealTraceImage()).into(sealImage);
			}
		};
	...
	private void getSpecialPointFileRequest(){
		...
		mPresenter.readyPresentServiceParams(that, url, mGetSpecialPointFileAction, rpc);
